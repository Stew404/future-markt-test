...
const isNotEmpty = (value) => {
    return value.length > 0 ? true : "Поле пустое";
};

const isValidPhoneNumber = (value) => {
    const re = /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/;

    return re.test(value) ? true : "Некорректный номер телефона";
};
...
const validate = (fields) => {
    ...
    for (const field of fields) {
        for (const validator of field.validators) {
            const validatorResult = validator(field.value);

            results.push(validatorResult);

            if (typeof validatorResult === "string") {
                field.setErrorMessage(validatorResult);
                break;
            }
...
export default function CallForm() {
    ...
    const handleSubmit = (e) => {
        ...
        const fields = [
            ...
            {
                value: isAgree,
                validators: [() => (isAgree ? true : "Галочка не нажата")],
                setErrorMessage: setIsAgreeError,
            },
        ];

        const validateResult = validate(fields);

        setIsValidated(validateResult);
    ...
    const callFromStyle = isCallFormOpen
        ? `${styles.callform} ${styles.callform_opened}`
        : styles.callform;

    const callFromOutsideStyle = isCallFormOpen
        ? `${styles.callform__outside} ${styles.callform__outside_opened}`
        : styles.ccallform__outside;

    return (
        <>
            <div
                className={callFromOutsideStyle}
                onClick={() => {
                    setIsCallFormOpen(false);
                }}
            ></div>
            <div className={callFromStyle}>
                <button
                ...
                {isValidated ? (
                    <>
                        <h2 className={styles.callform__thankheading}>
                            Спасибо за заявку
                        </h2>
                        <p className={styles.callform__thankmessage}>
                            Я обязательно свяжусь с вами в ближайшее время.
                        </p>
                        <svg
                            className={styles.callform__thanklogo}
                            width="170"
                            height="34"
                            viewBox="0 0 170 34"
                            fill="none"
                        ...
                        >
                            ...
                            />
                            <path
                                d="M15.3512 24H24.5113V21.9395H17.9977V9.78125H15.3512V24Z"
                                fill="white"
                            />
                            <path
                    ...
                    <>
                        ...
                        <form className={styles.form} onSubmit={handleSubmit}>
                            ...
                            <div className={styles.form__agreement}>
                                <label
                                    className={styles.form__agreementinput}
